...
import com.o2d.pkayjava.editor.view.ui.followers.PolygonTransformationListener;
...
public class PolygonFollower extends SubFollower {
    ...
    public void drawOutlines() {
        if (drawPoints.length > 0) {
            ...
            for (int i = 1; i < drawPoints.length; i++) {
                ...
                shapeRenderer.line(drawPoints[i].x * transformComponent.getScaleX(), drawPoints[i].y * transformComponent.getScaleY(), drawPoints[i - 1].x * transformComponent.getScaleX(), drawPoints[i - 1].y * transformComponent.getScaleY());
            }
            shapeRenderer.setColor(outlineColor);
            ...
            shapeRenderer.line(drawPoints[drawPoints.length - 1].x * transformComponent.getScaleX(), drawPoints[drawPoints.length - 1].y * transformComponent.getScaleY(), drawPoints[0].x * transformComponent.getScaleX(), drawPoints[0].y * transformComponent.getScaleY());
            shapeRenderer.end();
        }
    ...
    public void drawTriangulatedPolygons() {
        ...
        for (Vector2[] poly : polygonComponent.vertices) {
            for (int i = 1; i < poly.length; i++) {
                shapeRenderer.line(poly[i - 1].x * transformComponent.getScaleX(), poly[i - 1].y * transformComponent.getScaleY(), poly[i].x * transformComponent.getScaleX(), poly[i].y * transformComponent.getScaleY());
            }
            if (poly.length > 0)
                shapeRenderer.line(poly[poly.length - 1].x * transformComponent.getScaleX(), poly[poly.length - 1].y * transformComponent.getScaleY(), poly[0].x * transformComponent.getScaleX(), poly[0].y * transformComponent.getScaleY());
        }
        shapeRenderer.end();
    }
    ...
    public void drawPoints() {
        for (int i = 0; i < originalPoints.size(); i++) {
            ...
            shapeRenderer.rect(originalPoints.get(i).x * transformComponent.getScaleX() - side / 2f, originalPoints.get(i).y * transformComponent.getScaleY() - side / 2f, side, side);
            shapeRenderer.setColor(Color.BLACK);
            shapeRenderer.rect(originalPoints.get(i).x * transformComponent.getScaleX() - side / 2f + onePixel, originalPoints.get(i).y * transformComponent.getScaleY() - side / 2f + onePixel, side - 2 * onePixel, side - 2 * onePixel);
            shapeRenderer.end();
        }
    }
        ...
        addListener(new ClickListener() {
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (anchorId >= 0) {
                    ...
                    listener.anchorDown(PolygonFollower.this, anchorId, x * runtimeCamera.zoom / transformComponent.getScaleX(), y * runtimeCamera.zoom / transformComponent.getScaleY());
                } else if (lineIndex > -1) {
                    // not anchor but line is selected gotta make new point
                    listener.vertexDown(PolygonFollower.this, lineIndex, x * runtimeCamera.zoom / transformComponent.getScaleX(), y * runtimeCamera.zoom / transformComponent.getScaleY());
                }
                return true;
            }
            ...
            public void touchDragged(InputEvent event, float x, float y, int pointer) {
                ...
                if (anchorId >= 0) {
                    listener.anchorDragged(PolygonFollower.this, anchorId, x * runtimeCamera.zoom / transformComponent.getScaleX(), y * runtimeCamera.zoom / transformComponent.getScaleY());
                } else if (lineIndex > -1) {
            ...
            public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (anchorId >= 0) {
                    listener.anchorUp(PolygonFollower.this, anchorId, x * runtimeCamera.zoom / transformComponent.getScaleX(), y * runtimeCamera.zoom / transformComponent.getScaleY());
                } else if (lineIndex > -1) {
                    listener.vertexUp(PolygonFollower.this, lineIndex, x * runtimeCamera.zoom / transformComponent.getScaleX(), y * runtimeCamera.zoom / transformComponent.getScaleY());
                }
                draggingAnchorId = -1;
            }
    ...
    private int vertexHitTest(float x, float y) {
        ...
        for (int i = 1; i < drawPoints.length; i++) {
            Vector2 pointOne = drawPoints[i - 1].cpy().scl(1f / runtimeCamera.zoom * transformComponent.getScaleX(), 1f / runtimeCamera.zoom * transformComponent.getScaleY());
            Vector2 pointTwo = drawPoints[i].cpy().scl(1f / runtimeCamera.zoom * transformComponent.getScaleX(), 1f / runtimeCamera.zoom * transformComponent.getScaleY());
            if (Intersector.intersectSegmentCircle(pointOne, pointTwo, tmpVector, circleSqr)) {
                lineIndex = i;
                break;
            }
        }
        Vector2 pointOne = drawPoints[drawPoints.length - 1].cpy().scl(1f / runtimeCamera.zoom * transformComponent.getScaleX(), 1f / runtimeCamera.zoom * transformComponent.getScaleY());
        Vector2 pointTwo = drawPoints[0].cpy().scl(1f / runtimeCamera.zoom * transformComponent.getScaleX(), 1f / runtimeCamera.zoom * transformComponent.getScaleY());
    ...
    private int anchorHitTest(float x, float y) {
        ...
        for (int i = 0; i < drawPoints.length; i++) {
            Circle pointCircle = new Circle(drawPoints[i].x / runtimeCamera.zoom * transformComponent.getScaleX(), drawPoints[i].y / runtimeCamera.zoom * transformComponent.getScaleY(), (float) CIRCLE_RADIUS / pixelsPerWU);
