...
public class TransformMathUtils {
    ...
    public static Vector2 parentToLocalCoordinates(Entity childEntity, Vector2 parentCoords) {
        ...
        final float rotation = trnasform.getRotation();
        final float scaleX = trnasform.getScaleX();
        final float scaleY = trnasform.getScaleY();
        final float childX = trnasform.getX();
        final float childY = trnasform.getY();
        if (rotation == 0) {
    ...
    public static Vector2 localToParentCoordinates(Entity entity, Vector2 localCoords) {
        ...
        final float x = transform.getX();
        final float y = transform.getY();
        if (rotation == 0) {
            if (scaleX == 1 && scaleY == 1) {
                localCoords.x += x;
                localCoords.y += y;
            } else {
                final float originX = transform.getOriginX();
                final float originY = transform.getOriginY();
