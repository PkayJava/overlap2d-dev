...
public abstract class ComponentFactory {
    ...
    protected TransformComponent createTransformComponent(Entity entity, MainItemVO vo, DimensionsComponent dimensionsComponent) {
        ...
        component.setRotation(vo.rotation);
        component.setScaleX(vo.scaleX);
        component.setScaleY(vo.scaleY);
        component.setX(vo.x);
        component.setY(vo.y);

        if (Float.isNaN(vo.originX)) {
            component.setOriginX(dimensionsComponent.width / 2f);
        } else {
            component.setOriginX(vo.originX);
        }

        if (Float.isNaN(vo.originY)) {
            component.setOriginY(dimensionsComponent.height / 2f);
        } else {
            component.setOriginY(vo.originY);
        }
